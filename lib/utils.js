...
var utils = {
  ...
  dateToDuration: function(dateStr) {
    var dur = {};
    var was = utils.extractDate(dateStr);
    if (!was) {
      return false;
    }
    dur.was = was;

    if (!utils.siteRelativeDate) {
  ...
  extractDate: function(dateStr) {
    ...
    var date = new Date(dateMatch[0].replace(/-/g, '/'));
  ...
  shortDurationFormat: function(dateStr, nowDateStr) {
    var was = utils.extractDate(dateStr);
    if (!was) {
      return false;
    }
    ...
    if (nowDateStr) {
      var now = utils.extractDate(nowDateStr);
    }
  ...
  getAuthkey: function() {
    ...
    var ak = gData.get(ext.modules.global.name, 'ak');
    this.authKey = ak;
    return ak;
  },
